...
class TokenBucket:
    ...
    def available(self, client_id: str, now: float | None = None) -> float:
        ...
        Trả về số token hiện có trong xô của client, đã cộng phần nạp lại
        kể từ lần cập nhật cuối nhưng không vượt quá ``capacity``.
        """
    ...
    def consume(self, client_id: str, now: float | None = None) -> None:
        ...
        Cập nhật cả ``tokens`` lẫn ``ts`` để lần tính nạp lại sau dùng
        đúng mốc thời gian vừa tiêu token.
        """
        now = now if now is not None else time.time()
